...
bool isvalidemail(const string& email) {
    ...
    return at_position != string::npos &&
           dot_position != string::npos &&
           at_position < dot_position;
}

bool roledetails(const string& role,
                 string& filename,
                 string& code_prefix,
...
bool authsystem::createaccountrecord(const string& role,
                                     const string& first_name,
                                     const string& last_name,
                                     const string& email_id,
                                     const string& password,
                                     const string& admin_key,
                                     person& created_person,
                                     string& error_message,
                                     const string& requested_code) {
    string filename;
    string code_prefix;
    bool require_admin_key = false;
    if (!roledetails(role, filename, code_prefix, require_admin_key)) {
        error_message = "role must be one of admin, user, or lib";
        return false;
    }

    if (require_admin_key && admin_key != admin_login_key) {
        error_message = "invalid admin key";
        return false;
    }

    if (first_name.empty() || last_name.empty() || email_id.empty() || password.empty()) {
        error_message = "fname, lname, email, and password are required";
        return false;
    }
    ...
    loadlist(people, filename);

    if (bsearchbyemail(people, email_id) != -1) {
        error_message = "email already registered";
        return false;
    }

    string final_code = requested_code;
    if (!final_code.empty()) {
        for (const person& current_person : people) {
            if (current_person.getcode() == final_code) {
                error_message = "code already assigned";
                return false;
            }
        }
    } else {
        final_code = code_prefix + to_string(people.size() + 1);
    }

    created_person.setdetails(first_name,
                              last_name,
                              email_id,
                              final_code);

    people.push_back(created_person);
    savelist(people, filename);
    storepassword(role, email_id, password);
    return true;
}

bool authsystem::loginrecord(const string& role,
                             const string& email_id,
                             const string& password,
                             const string& admin_key,
    ...
    if (email_id.empty() || password.empty()) {
    ...
    int index = bsearchbyemail(people, email_id);
    if (index == -1) {
        error_message = role + " account not found";
        return false;
    }

    if (!checkpassword(role, email_id, password)) {
        error_message = "invalid credentials";
        return false;
    }

    matched_person = people[index];
    return true;
}

vector<person> authsystem::loadrolepeople(const string& role) {
    string filename;
    string code_prefix;
    bool require_admin_key = false;
    vector<person> people;
